...
public class ParmsBayless extends Bayless {
	...
	getSupportActionBar().hide();
        setContentView(R.layout.parmsbayless);
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Bayless/JH-Bayless.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Bayless/JH-Bayless.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Bayless.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Bayless.par "+getFilesDir()+"/Bayless");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Bayless.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Bayless.dat "+getFilesDir()+"/Bayless");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsBayless.this, Bayless.class);
