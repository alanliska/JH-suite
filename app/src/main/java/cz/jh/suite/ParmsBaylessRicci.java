...
public class ParmsBaylessRicci extends BaylessRicci {
	...
	getSupportActionBar().hide();
        setContentView(R.layout.parmsbaylessricci);
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Bayless-Ricci/JH-Bayless-Ricci.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Bayless-Ricci/JH-Bayless-Ricci.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Bayless-Ricci.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Bayless-Ricci.par "+getFilesDir()+"/Bayless-Ricci");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Bayless-Ricci.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Bayless-Ricci.dat "+getFilesDir()+"/Bayless-Ricci");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsBaylessRicci.this, BaylessRicci.class);
