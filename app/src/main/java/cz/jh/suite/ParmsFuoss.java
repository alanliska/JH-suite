...
public class ParmsFuoss extends Fuoss {
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Fuoss/JH-Fuoss.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Fuoss/JH-Fuoss.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Fuoss.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Fuoss.par "+getFilesDir()+"/Fuoss");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Fuoss.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Fuoss.dat "+getFilesDir()+"/Fuoss");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsFuoss.this, Fuoss.class);
