...
public class ParmsMarcus extends Marcus {
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Marcus/JH-Marcus.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Marcus/JH-Marcus.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Marcus.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Marcus.par "+getFilesDir()+"/Marcus");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Marcus.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Marcus.dat "+getFilesDir()+"/Marcus");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsMarcus.this, Marcus.class);
