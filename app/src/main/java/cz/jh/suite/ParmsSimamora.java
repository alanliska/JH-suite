...
public class ParmsSimamora extends Simamora {
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Simamora/JH-Simamora.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Simamora/JH-Simamora.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Simamora.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Simamora.par "+getFilesDir()+"/Simamora");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Simamora.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Simamora.dat "+getFilesDir()+"/Simamora");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsSimamora.this, Simamora.class);
