...
public class ParmsZhou extends Zhou {
    ...
    {
        super.onStart();
        ParDisplay(exec("cat "+getFilesDir()+"/Zhou/JH-Zhou.par"));
        DatDisplay(exec("cat "+getFilesDir()+"/Zhou/JH-Zhou.dat"));
    }
    ...
    private View.OnClickListener saveFileClick; {
        saveFileClick = new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream fileout10 = openFileOutput("JH-Zhou.par", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout10);
                    outputWriter.write(ParFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Zhou.par "+getFilesDir()+"/Zhou");

                String DatFile = DatContent.getText().toString();
                // TODO Auto-generated method stub //
                try {
                    FileOutputStream fileout20 = openFileOutput("JH-Zhou.dat", MODE_PRIVATE);
                    OutputStreamWriter outputWriter = new OutputStreamWriter(fileout20);
                    outputWriter.write(DatFile);
                    outputWriter.close();
                } catch (Exception e) {
                }
                exec("mv "+getFilesDir()+"/JH-Zhou.dat "+getFilesDir()+"/Zhou");
            }
        };
    }
    ...
    private View.OnClickListener QuitClick; {
        QuitClick = new View.OnClickListener() {
            public void onClick(View v) {
                // TODO Auto-generated method stub //
                Intent intent = new Intent(ParmsZhou.this, Zhou.class);
